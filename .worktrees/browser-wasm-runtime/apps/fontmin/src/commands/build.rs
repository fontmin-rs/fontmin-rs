...
use fontmin::{
    Asset, CssOptions, CssPlugin, CssTarget, FontFormat, OutputFormat, Svgs2TtfOptions,
    Svgs2TtfPlugin,
};
use fontmin_config::{
    CssConfig, CssTarget as ConfigCssTarget, FontminConfig, OutputConfig, SubsetConfig,
};
...
use super::{format::parse_output_formats, unicode::parse_optional_unicodes};
...
fn apply_cli_overrides(config: &mut FontminConfig, options: BuildOptions) -> Result<()> {
    ...
    if options.font_family.is_some() || options.font_path.is_some() || options.css_glyph {
