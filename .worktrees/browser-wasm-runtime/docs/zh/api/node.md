...
在浏览器中处理字体时，请安装 `@fontmin-rs/wasm`。先初始化一次，再将具名的内存资产传给 `optimizeBrowser()`：
...
浏览器包提供异步转换、检查与内存内置 pipeline；它不接受路径、glob、CLI 选项、磁盘缓存或 Node 文件系统插件 hooks。
...
`modernWeb()` 等价于组合 `glyph()`、`ttf2woff()`、`ttf2woff2()` 和 `css()`。它不会生成 EOT 或 SVG；如需这些格式，请显式添加 `ttf2eot()` 或 `ttf2svg()`。
