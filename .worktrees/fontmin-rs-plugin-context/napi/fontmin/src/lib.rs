use fontmin::{
    CssFontSource, CssOptions, EotOptions, FontFormat, FontInfo, FontMetadata, LayoutSubsetMode,
    Otf2TtfOptions, OutputFormat, SubsetOptions, Svg2TtfOptions, SvgIcon, Svgs2TtfOptions,
...
pub fn generate_font_face_css(
    ...
    let css = fontmin::generate_font_face_css(&sources, &css_options_from_js(options))
        .map_err(|error| napi::Error::from_reason(error.to_string()))?;
...
fn css_sources_from_js(sources: Vec<JsCssFontSource>) -> napi::Result<Vec<CssFontSource>> {
    sources
        .into_iter()
        .map(|source| {
            Ok(CssFontSource::new(
                source.file_name,
                output_format_from_js(&source.format)?,
            ))
        })
        .collect()
}

fn css_options_from_js(options: Option<JsCssOptions>) -> CssOptions {
    let Some(options) = options else {
        return CssOptions::default();
    };
