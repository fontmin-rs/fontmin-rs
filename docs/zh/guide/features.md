...
fontmin-rs 目前处于 `0.1.0` RC 周期。本页集中记录已冻结的文档化公开能力，以及选择不同入口时需要了解的边界。第一次使用时，可以继续阅读[快速开始](./getting-started)。
...
当前开发重点是发布加固、改进 diagnostics、性能优化，以及覆盖更多 Fontmin 兼容能力。稳定版本的具体准入条件记录在[首个稳定版路线图](../roadmap)中，更深层的设计背景仍保留在[设计文档](../../fontmin-rs-design)；维护者可以使用[发布准备清单](../../releasing)检查候选版本是否达到发布门槛。
