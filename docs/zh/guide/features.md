...
fontmin-rs 已进入稳定的 `0.1` 发布线。本页集中记录文档化公开能力，以及选择不同入口时需要了解的边界。第一次使用时，可以继续阅读[快速开始](./getting-started)。
...
当前开发重点是发布加固、改进 diagnostics、性能优化，以及覆盖更多 Fontmin 兼容能力。具体里程碑记录在[迈向 1.0 的路线图](../roadmap)中，当前边界以[项目架构](../architecture)为准；维护者可以使用[发布准备清单](../../releasing)检查每个版本是否达到发布门槛。
