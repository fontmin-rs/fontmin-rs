...
fontmin-rs `0.1.0` 已作为首个稳定版本发布，覆盖 CLI、Node.js 包、浏览器 WASM
包、native binding 和 8 个平台包。后续里程碑会在保持公开契约的前提下加深内部
实现，并在独立验证后最终确定 `1.0.0` 契约。
...
没有记录未解决的 P0/P1 问题。稳定版提升还会拒绝 RC 标签之后的运行时代码变更。

退出条件：独立版本的 RC 契约通过完整发布门禁和真实项目验证，同一提交无需发布时修补
即可提升为 `1.0.0`。
