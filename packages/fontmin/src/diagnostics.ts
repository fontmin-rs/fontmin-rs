...
export function normalizeFontminDiagnostic(error: unknown): Error {
  ...
  if (match === null || match === undefined) {
    ...
    return new Error('fontmin-rs operation failed', { cause: error })
  }
  ...
  if (code === undefined || diagnosticMessage === undefined) {
    return new Error('fontmin-rs operation returned an invalid diagnostic', {
      cause: error,
    })
  }
...
export function withFontminDiagnostics<T>(operation: () => T): T {
  try {
    return operation()
  } catch (error) {
