...
fn read_index<'a>(ctxt: &mut ReadCtxt<'a>, count: usize) -> Result<Index<'a>, ParseError> {
    if count > 0 {
        ...
        let offset_count = count.checked_add(1).ok_or(ParseError::BadValue)?;
        let offset_array_size = offset_count
            .checked_mul(usize::from(off_size))
            .ok_or(ParseError::BadValue)?;
        let offset_array = ctxt.read_slice(offset_array_size)?;

        let first_offset = lookup_offset_index(off_size, offset_array, 0);
        if first_offset != 1 {
            return Err(ParseError::BadValue);
        }
        ...
        for index in 1..offset_count {
            let offset = lookup_offset_index(off_size, offset_array, index);
        ...
        let data_array_size = previous_offset.checked_sub(1).ok_or(ParseError::BadValue)?;
